...
#  event.bot.send_msg({:text => "Hi"})
  text = sender["message"]["text"]
#  
#  member = BotMember.where(:name => text).take
#  if member != nil
#    event.bot.send_msg({:text => "Hi, #{member.name}"})
#  else
#    event = FBMessage.new(sender)
#    event.bot.send_msg({:text => "I can't find that person"})
#  end

  broadcast(text)
#  event = FBMessage.new(sender)
#  event.bot.send_msg({:text => "#{@person1.name} #{@person.name2} #{@connected}"})
...
private
def broadcast(msg)
  members = BotMember.all
  members.each do |member|
    sender = {"sender" => {"id" => member.sender_id}}
    event = FBMessage.new(sender)
    event.bot.send_msg({:text => msg})
  end
end
